...
class AttackerClientMultiTargetNonIID(object):
    def __init__(self, client_id, dataset_name, train_dataset, train_loader, attacker_dataset,
                 ...
                 scale_rate, global_pruning, use_adaptive, adaptive_lr, device, scaled=False, all_num_clients=5, ema_scale=0.9999):
        self.dataset_name = dataset_name
        if dataset_name == 'cifar10':
            self.img_size = 32
        elif dataset_name == 'celeba':
    ...
    def local_train(self, round, local_epoch, mid_T, use_labels=True):
        ...
        if self.use_model_poison or self.use_critical_poison > 0:
            ...
            if self.scaled:
                ...
                new_scale_rate = self.scale_rate

            for key, value in self.global_model.state_dict().items():
                    target_value = self.target_model.state_dict()[key]
                    new_value = target_value + (value - target_value) * new_scale_rate
                    self.global_model.state_dict()[key].copy_(new_value)
...
class AttackerClientMultiTargetNonIIDLayerSub(object):
    ...
    def __init__(self, client_id, dataset_name, train_dataset, train_loader, attacker_dataset, 
                 attacker_loader, use_model_poison, use_pgd_poison, use_critical_poison, critical_proportion,
                 use_bclayersub_poison, scale_rate, device, scaled=False, ema_scale=0.9999):
        self.dataset_name = dataset_name
        if dataset_name == 'cifar10':
            self.img_size = 32
        elif dataset_name == 'celeba':
...
class ClientsGroupMultiTargetAttackedNonIID(object):
    ...
    def __init__(self, dataset_name, batch_size, clients_num, num_targets, device, scaled=False,
                 batch_size_attack_per=0.5, scale_rate=1, use_model_poison=False, defense_technique=None,
    ...
    def data_allocation(self):
        ...
        for i in range(self.clients_num):
            ...
            if i < 2 and self.use_no_poison == False:
                ...
                if self.scaled:
                    train_dataset_benign, dataset_attacker = data_allocation_attacker_noniid(client_data_idxs, \
                                                                                  ...
                                                                                  save_targets_path=f'./images/targets_{self.num_targets}_fl_{formatted_time}_client_{i}_seed_{self.data_distribution_seed}_{self.defense_technique}_two_attackers_scaled',
                    ...
                    train_dataset_benign, dataset_attacker = data_allocation_attacker_noniid(client_data_idxs, \
                                                                                  attack_batch_size=attack_batch_size,
                                                                                  num_targets=self.num_targets, \
                                                                                  save_targets_path=f'./images/targets_{self.num_targets}_fl_{formatted_time}_client_{i}_seed_{self.data_distribution_seed}_{self.defense_technique}_two_attackers',
                                                                                  seed=self.data_distribution_seed,
                                                                                  dataset_name=self.dataset_name)
                ...
                if self.use_layer_substitution:
                    client = AttackerClientMultiTargetNonIIDLayerSub(i, self.dataset_name, train_dataset_benign, train_loader_client, 
                                    ...
                                    self.scale_rate, self.device, self.scaled, ema_scale=self.ema_scale)
                else:
                    client = AttackerClientMultiTargetNonIID(i, self.dataset_name, train_dataset_benign, train_loader_client, 
                                    ...
                                    self.device, self.scaled, all_num_clients=self.all_num_clients, ema_scale=self.ema_scale)
            else:
                train_loader_client = DataLoader(

...
from clients_fed_coordinated import ClientsGroupMultiTargetAttackedNonIID, AttackerClientMultiTargetNonIID
...
flags.DEFINE_integer('adaptive_k', 50, help='Indicator magnification factor for AdaSCP.')
...
def train():
    ...
    for round in tqdm(range(0, FLAGS.total_round)):
        ...
        # After all attacker clients have called local_train for this round:
        clients_group.colluders_shared_state.aggregate(
                adaptive_k=FLAGS.adaptive_k,
                all_num_clients=FLAGS.client_num
        )
        # Then, each attacker client can call prepare_for_next_round() before their next local_train
        # Let colluding clients prepare for the next round
        logging.info(f"[Main Train Loop] Round {round}: Colluders preparing for next round (fetching tars_coll).")
        for c_idx in range(clients_group.colluding_clients): # Iterate only over colluding clients
            if isinstance(clients_group.clients_set[c_idx], AttackerClientMultiTargetNonIID):
                clients_group.clients_set[c_idx].prepare_for_next_round() # This calls prepare_for_collusion_round

        # Reset shared state *after* all colluders have fetched the aggregated result for this round
        clients_group.colluders_shared_state.reset()
        logging.info(f"[Main Train Loop] Round {round}: Shared state reset for next round's submissions.")
        ...
        if FLAGS.save_round > 0 and (round+1) % FLAGS.save_round == 0:
            ...
            datetime_str = time.strftime("%Y-%m-%d-%H-%M", time.localtime())
            filename = f"global_ckpt_round{round+1}_{datetime_str}.pt"
            torch.save(global_ckpt, os.path.join(global_model_path, filename))
